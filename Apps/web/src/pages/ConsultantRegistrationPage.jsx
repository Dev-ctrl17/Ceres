...
const fieldClass =
  "mt-1 w-full rounded-md border border-slate-200 bg-slate-100 px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100";
...
function getRegistrationErrorMessage(errorCode) {
  return (
    registrationErrorMessages[errorCode] ||
...
export default function ConsultantRegistrationPage() {
  ...
  async function submit(event) {
    ...
    if (!TURNSTILE_SITE_KEY)
      return setError(
        "Turnstile is not configured in Vercel yet. Add VITE_TURNSTILE_SITE_KEY, redeploy, then try again.",
      );
    if (!turnstileToken)
      return setError("Please complete the security check before submitting.");
    if (form.email !== form.confirm_email)
      return setError("Email addresses do not match.");
    if (isTeamLeader && form.password.length < 8)
      return setError("Password must be at least 8 characters.");
    if (isTeamLeader && form.password !== form.confirm_password)
      return setError("Passwords do not match.");
    if (!form.accepted_terms)
      return setError("Please agree to the terms and conditions.");
    setLoading(true);
    const { confirm_email, confirm_password, accepted_terms, first_name, last_name, password, ...payload } =
      form;
    const body = {
      ...payload,
    ...
    setLoading(false);
    const errorCode =
      data?.error || (await getInvocationErrorCode(invokeError));
    ...
    <>
      ...
      <main className="min-h-screen bg-white py-10 sm:py-16">
        <section className="mx-auto w-full max-w-4xl px-4">
          ...
          {referralCode && (
            <div className="mb-8 flex items-center justify-between rounded-md bg-amber-500 px-5 py-3 text-center text-xs font-medium text-white">
            ...
            {result ? (
              <div className="py-12 text-center">
                ...
                <p className="mt-3 text-sm text-slate-500">
                  {isTeamLeader
                    ? "Your registration is complete. You can log in at /consultant-portal with the email address and password you set."
                    : "Your referral link is ready."}
                </p>
                <input
                  readOnly
                  value={result.referralLink}
                  className="mt-6 w-full rounded border bg-slate-50 p-3 text-sm"
                />
              </div>
            ) : (
              <form onSubmit={submit} className="space-y-4">
                ...
                <button
                  disabled={loading}
                  className="w-full rounded-md bg-amber-500 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50"
                >
