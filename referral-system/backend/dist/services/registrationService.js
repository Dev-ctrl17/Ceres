 ...
 */
async function registerConsultant(input, supabase) {
    // --------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------
    const { data: newUserId, error: rpcError } = await supabase.rpc('register_consultant', {
    ...
    if (rpcError) {
        ...
        if (message.includes('PHONE_TAKEN'))
            throw new Error('PHONE_TAKEN');
        if (message.includes('INVALID_REF_CODE'))
            throw new Error('INVALID_REF_CODE');
    ...
    // --------------------------------------------------------------------------
    const referralLink = `${types_1.REFERRAL_BASE_URL}?ref=${newUser.referral_code}`;
    const whatsappMessage = `🏠 *Join Luxury Properties Ltd — Earn on Every Referral!*\n\n` +
